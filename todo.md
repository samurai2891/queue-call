...
## M11: アクセスキー削除とキオスクQRトークン導入

### 背景・目的
- アクセスキーがエラーの原因になるため削除
- 呼び出しボードは荒らされないのでキー不要
- キオスクはQR URLトークンで荒らし対策（再生成で過去URLを無効化）

### Phase 1: データベーススキーマ更新
- [x] 1-1: storesテーブルにkioskTokenカラム追加
- [x] 1-2: boardKeyカラムを削除（または無視）
- [x] 1-3: マイグレーション実行

### Phase 2: サーバー側API更新
- [x] 2-1: キオスクトークン再生成API追加
- [x] 2-2: キオスク表示画面のアクセス検証をトークンベースに変更
- [x] 2-3: ボード表示画面のアクセス検証を削除

### Phase 3: キオスク管理画面更新
- [x] 3-1: QR URLトークン再生成ボタン追加
- [x] 3-2: 再生成時の確認ダイアログ（過去URLが無効になる警告）
- [x] 3-3: 現在のQR URLとQRコード表示
...
### Phase 5: 表示画面更新
- [x] 5-1: KioskDisplay.tsxのアクセス検証をトークンベースに変更
- [x] 5-2: BoardDisplay.tsxのアクセス検証を削除

### Phase 6: テスト・完了
- [x] 6-1: 動作確認（59件のテスト全パス）
- [x] 6-2: チェックポイント保存

### 新しいURL構造
- キオスク: `/s/:storeSlug/kiosk/display?token=xxx`（トークン必須、再生成可能）
