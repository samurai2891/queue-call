...
## 新規タスク: プリペイドモデル + マージンでSMS通知実装

### フェーズ1: Stripe統合とDBスキーマ更新
- [x] webdev_add_feature で Stripe 統合を追加
- [x] stores テーブルに smsBalance カラム追加
- [x] smsTransactions テーブル追加（チャージ・消費履歴）
- [x] マイグレーション実行

### フェーズ2: SMS残高管理とチャージUI実装
- [x] Stripe Checkout セッション作成API
- [x] Stripe Webhook 処理（残高更新）
- [x] 設定画面にチャージUI追加（残高表示・チャージボタン）
- [x] チャージ成功/失敗の通知

### フェーズ3: SMS送信機能と残高引き落とし
- [x] Twilio SMS送信関数の実装
- [x] SMS送信前の残高チェック
- [x] SMS送信成功時の残高引き落とし（20円/通）
- [x] 残高不足時はWeb Pushのみ送信
- [x] 残高アラート通知（1,000円以下で警告）
...
### 確認ポイント (DoD)
- [x] 店舗がStripeでチャージできる
- [x] SMS送信ごとに残高から20円引き落とされる
- [x] 残高不足時は自動的にWeb Pushのみになる
- [x] 残高が少なくなったらアラート通知される
- [ ] 顧客がSMS通知を登録できる
...
### 1. 顧客側SMS登録フロー（チケット画面）
- [x] チケット画面にSMS通知登録セクションを追加
- [x] 電話番号入力フォーム（国番号選択付き）
- [x] OTP送信API（Twilio Verify）の呼び出し
- [x] OTP入力フォームと検証
- [x] SMS登録完了状態の表示
- [x] SMS登録解除機能

### 2. 自動チャージ促進機能
- [x] 残高が閾値以下になった際のアラート表示
- [x] チャージ促進モーダル/バナーの実装
- [x] 設定画面で閾値を設定可能にする
- [x] SMS送信失敗時（残高不足）のエラーメッセージ表示
...
- [x] 顧客がチケット画面からSMS通知を登録できる
- [x] OTP認証が正常に動作する
- [x] 残高が少なくなったらチャージを促される
- [x] 管理者がSMS送信履歴を確認できる
