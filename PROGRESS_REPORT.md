...
| M3: 店舗設定ページ | 90% | 🟡 ほぼ完了 |
| M4: メニュー＋画像アップロード | 60% | 🟡 部分実装 |
| M5: スタッフの順番調整 | 30% | 🔴 部分実装 |
| M6: i18n漏れゼロ | 80% | 🟡 ほぼ完了 |
...
### T3-3. キオスク/ボードのURL発行 🟡
- 🟡 /kiosk?key=... と /board?key=... で key必須（部分実装）
- ❌ key再生成で旧keyは無効化
- ❌ 店舗設定画面に「URLをコピー」ボタン

---

## 🟡 M4: メニュー＋画像アップロード（60%完了）
...
### T4-3. 店舗側のメニュー管理画面 🟡
- 🟡 フィード投稿：追加/削除/並び替え/ONOFF（部分実装）
- 🟡 メニュー一覧：追加/削除/ONOFF、画像、値段（部分実装）
...
**最優先残タスク:**
- presigned URL による画像アップロード機能の実装
